  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <h1 className="text-lg font-bold text-gray-900 mt-1 break-all">
          {page.url}
        </h1>
      ...
      <Card
        title="기본 메타 태그"
        subtitle="검색엔진이 페이지를 이해하는 데 사용하는 핵심 태그"
        delay={0.2}
      >
      ...
      {/* Verification tags */}
      {data.metadata?.verificationTags &&
        Object.keys(data.metadata.verificationTags).length > 0 && (
          <Card delay={0.28}>
            <div className="flex items-center gap-2 mb-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          items={[
            ...
            .map((tag) => ({
              name: `<${tag}>`,
              value: data.semantic?.semanticTags?.[tag] || 0,
            }))
            .filter((i) => i.value > 0)}
          delay={0.35}
        />
        ...
        <StatCard
          ...
          items={[
            ...
            {
              name: "Alt 비율",
              value: `${Math.round((data.semantic?.images?.altRatio || 0) * 100)}%`,
            },
          ]}
          delay={0.41}
        />
      ...
      {data.keywords?.density?.length > 0 &&
        (() => {
          ...
          return (
            <Card
              title="키워드 밀도"
              subtitle={`총 ${data.keywords.totalText?.chars?.toLocaleString()}자 · ${data.keywords.totalText?.words?.toLocaleString()}단어`}
              delay={0.47}
            >
              <div style={{ height: chartHeight }}>
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    />
                    <Tooltip
                      formatter={(value: any) => [`${value}회`, "빈도"]}
                    />
                    <Bar
                      dataKey="count"
                      fill="#6366F1"
