  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            {session.url} · {new Date(session.created_at).toLocaleString('ko-KR')}
      ...
      {isSinglePage ? (
        ...
        <MultiPageReport session={session} pages={pages} successPages={successPages} sessionId={sessionId!} />
      )}
    </div>
  );
}
...
function SinglePageReport({ page, session }: { page: any; session: any }) {
  ...
  const altRatio = data.semantic?.images?.total > 0
    ? Math.round(data.semantic.images.altRatio * 100) : 100;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard label="SEO 설정 완성도" value={score} sub="/100점" color="indigo" delay={0}
        ...
        <KpiCard label="타겟 키워드" value={targetKws.length} sub={targetKws.slice(0, 3).join(', ') || '-'} color="amber" delay={0.05}
        ...
        <KpiCard label="JSON-LD" value={jsonLdCount > 0 ? `${jsonLdCount}개` : '없음'} sub={jsonLdCount > 0 ? data.structuredData.jsonLd.map((l: any) => l.type).join(', ') : '구조화 데이터 미설정'} color={jsonLdCount > 0 ? 'emerald' : 'rose'} delay={0.1}
        ...
        <KpiCard label="이미지 Alt" value={`${altRatio}%`} sub={`${data.semantic?.images?.withAlt || 0}/${data.semantic?.images?.total || 0}개 설정됨`} color={altRatio >= 80 ? 'emerald' : 'rose'} delay={0.15}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card title="SEO 설정 완성도 레이더" subtitle="각 항목별 달성률 (%)" delay={0.2}>
          <div className="h-72">
            {radarData.length > 0 ? (
            ...
            ) : <p className="text-sm text-gray-400 text-center pt-12">데이터 없음</p>}
          </div>
        </Card>

        <Card title="항목별 점수" subtitle="SEO 설정 완성도 세부 항목" delay={0.25}>
          <div className="space-y-3 pt-1">
            {(data.scoreBreakdown || []).map((item: any, i: number) => (
              <div key={item.name} className="flex items-center gap-3">
                ...
                <span className="text-xs font-semibold text-gray-600 w-12 text-right">{item.score}/{item.maxScore}</span>
                <span className="text-[11px] text-gray-400 w-16 truncate text-right">{item.details}</span>
              </div>
            ))}
          </div>
        </Card>
      ...
      {/* Meta tags — basic */}
      <Card title="기본 메타 태그" subtitle="검색엔진이 페이지를 이해하는 핵심 태그" delay={0.3}>
      ...
      {/* JSON-LD + Headings */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card title="구조화 데이터 (JSON-LD)" delay={0.39}>
          <InfoTooltip text="검색엔진이 페이지의 콘텐츠 유형(리뷰, FAQ, 조직 정보 등)을 정확히 이해하도록 돕는 데이터입니다" />
          <div className="mt-3">
            <JsonLdViewer data={data.structuredData?.jsonLd || []} />
          </div>
        </Card>
        <Card title="Heading 구조" delay={0.42}>
          <InfoTooltip text="H1~H6 태그의 계층 구조입니다. H1은 페이지당 1개, 논리적 순서가 중요합니다" />
          <div className="mt-3">
      ...
      {/* Keywords */}
      {kwData.length > 0 && (
        <Card title="키워드 빈도 분포" subtitle={`본문 ${data.keywords?.totalText?.chars?.toLocaleString()}자 기준 상위 키워드`} delay={0.45}>
      ...
      {/* Links & Content stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatMiniCard label="내부 링크" value={data.semantic?.links?.internal || 0} delay={0.48} />
        <StatMiniCard label="외부 링크" value={data.semantic?.links?.external || 0} delay={0.5} />
        <StatMiniCard label="이미지" value={data.semantic?.images?.total || 0} delay={0.52} />
        <StatMiniCard label="렌더링" value={data.technical?.renderingType || 'unknown'} delay={0.54}
          tooltip={data.technical?.renderingType === 'SSR' ? '서버 렌더링 — 검색엔진에 유리' : '클라이언트 렌더링 — SEO에 불리할 수 있음'} />
      </div>
...
/* =========================================================
   ...
   ========================================================= */
function MultiPageReport({ session, pages, successPages, sessionId }: { session: any; pages: any[]; successPages: any[]; sessionId: string }) {
  const [activeTab, setActiveTab] = useState<'overview' | 'pages'>('overview');
  const summary = session.summary || {};
  const navigate = useNavigate();

  // Score distribution
  const scoreDistribution = successPages.map((p: any) => ({
  ...
  return (
    <div>
      <div className="flex gap-1 mb-6 bg-gray-100 rounded-xl p-1 w-fit">
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            ...
            {activeTab === tab.id && <motion.div layoutId="multiTab" className="absolute inset-0 bg-white rounded-lg shadow-sm" transition={{ type: 'spring', bounce: 0.15, duration: 0.4 }} />}
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <KpiCard label="평균 SEO 완성도" value={summary.avgScore || 0} sub="/100점" color="indigo" delay={0}
              icon={<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>} />
            <KpiCard label="분석 페이지" value={successPages.length} color="emerald" delay={0.05}
            ...
            <KpiCard label="주요 키워드" value={summary.topKeywords?.length || 0} color="amber" delay={0.1}
              icon={<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>} />
            <KpiCard label="JSON-LD 타입" value={Object.keys(summary.jsonLdTypes || {}).length} color="rose" delay={0.15}
              icon={<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>} />
          </div>
              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={scoreDistribution} margin={{ left: -10, right: 10, top: 5 }}>
                    ...
                    <XAxis dataKey="url" tick={{ fontSize: 10, fill: '#94A3B8' }} angle={-30} textAnchor="end" height={70} />
            ...
            <Card title="페이지 유형" delay={0.25}>
              <div className="h-56 flex items-center justify-center">
                {pageTypePie.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={pageTypePie} cx="50%" cy="50%" innerRadius={45} outerRadius={75} paddingAngle={4} dataKey="value">
            ...
            <Card title="사이트 전체 주요 키워드" delay={0.3}>
              <div className="flex flex-wrap gap-2">
                {summary.topKeywords.map((kw: any, i: number) => (
                  <span key={i} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-sm font-medium">
                    {kw.keyword} <span className="text-[11px] text-indigo-400">({kw.count})</span>
                  </span>
                ))}
              </div>
            </Card>
      ...
      {activeTab === 'pages' && (
        <Card noPadding delay={0.1}>
          <div className="divide-y divide-gray-50">
            {pages.sort((a: any, b: any) => (b.seo_data?.score || 0) - (a.seo_data?.score || 0)).map((page: any, i: number) => {
              const s = page.seo_data?.score;
              return (
                <motion.div key={page.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                  ...
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold ${
                    s >= 80 ? 'bg-emerald-50 text-emerald-600' : s >= 50 ? 'bg-amber-50 text-amber-600' : s != null ? 'bg-red-50 text-red-500' : 'bg-gray-50 text-gray-400'
                  }`}>{s ?? '-'}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">{page.url}</p>
                    <p className="text-[11px] text-gray-400 mt-0.5">{page.page_type || '기타'} · 깊이 {page.depth}</p>
                  </div>
...
function MetaItem({ label, value, badge }: { label: string; value?: string; badge?: string }) {
  return (
    <div className="flex items-start gap-2 py-2 border-b border-gray-50 last:border-0">
      ...
      <p className="flex-1 text-sm text-gray-800 break-all min-w-0">{value || <span className="text-gray-300">-</span>}</p>
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="flex items-center gap-2 py-4 px-3 bg-gray-50 rounded-xl">
      <svg className="w-4 h-4 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M20 12H4" /></svg>
      <span className="text-xs text-gray-400">{text}</span>
    </div>
  );
}

function StatMiniCard({ label, value, delay, tooltip }: { label: string; value: any; delay: number; tooltip?: string }) {
  return (
    <motion.div
      ...
      <p className="text-[11px] text-gray-400 mb-1 flex items-center gap-1">
        ...
        {tooltip && (
          <span className="relative">
            <span className="inline-flex items-center justify-center w-3.5 h-3.5 text-[9px] bg-gray-100 text-gray-400 rounded-full cursor-help">?</span>
            <span className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 px-2.5 py-1.5 bg-gray-900 text-white text-[10px] rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50">{tooltip}</span>
          </span>
        )}
      </p>
  ...
  return (
    <div className="bg-gray-900 text-white text-xs rounded-lg px-3 py-2 shadow-lg">
      ...
      {payload.map((e: any, i: number) => (
        <p key={i} className="text-gray-300">{e.name}: <span className="text-white font-semibold">{e.value}</span></p>
      ))}
    </div>
  );
}
